...
def home():
    event_name = eval(requests.get('http://name-api:5000/get_name').text)
    unit_type = eval(requests.get('http://unit-api:5000/get_unit').text)
    effect = requests.post('http://effect-api:5000/get_effect', json = {"event": event_name, "unit": unit_type})
    event = Events(event_name = event_name, unit_type = unit_type, status_effect = eval(effect.text), date_generated = date.today())
